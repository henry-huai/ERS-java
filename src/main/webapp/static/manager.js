...
.then(data=>{
    ...
    for(let request of data){
        ...
        if(request.status > 0){
            tableRow.innerHTML=`<td style="text-align:center">${rowIndex}</td><td style="text-align:center">${request.description}</td><td>Approved</td><td style="text-align:center">${request.request_id}</td><td>${request.category}</td><td>${request.transaction_date}</td><td style="text-align:center">${request.user_id}</td><td style="text-align:center">${request.status}</td>`
        }else{
            tableRow.innerHTML=`<td style="text-align:center">${rowIndex}</td><td style="text-align:center">${request.description}</td><td>Denied</td><td style="text-align:center">${request.request_id}</td><td>${request.category}</td><td>${request.transaction_date}</td><td style="text-align:center">${request.user_id}</td><td style="text-align:center">${-request.status}</td>`
        }
...
.then(data=>{
    ...
    for(let request of data){
        ...
        tableRow.innerHTML=`<td style="text-align:center">${rowIndex}</td><td style="text-align:center">${request.description}</td><td>Pending</td><td style="text-align:center">${request.request_id}</td><td>${request.category}</td><td>${request.transaction_date}</td><td style="text-align:center">${request.user_id}</td>
        <td>
        <div class="dropdown">
